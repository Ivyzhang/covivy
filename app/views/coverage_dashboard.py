...
def semantic_context_ranges(
        ...
        if tree is not None:
            for node in ast.walk(tree):
                ...
                if start is not None and end is not None:
                    python_nodes.append((start, end))
    for line, (window_start, window_end) in windows.items():
        containing_nodes = [
            (start, end)
            for start, end in python_nodes
            if start <= line <= end and start >= window_start and end <= window_end
        ]
        if containing_nodes:
    ...
    max_hunk_lines = context * 2 + 1
    for start, end in ranges:
        if not compacted or start > compacted[-1][1] + 1:
            compacted.append((start, end))
        else:
            compacted[-1] = (compacted[-1][0], max(compacted[-1][1], end))
    capped = []
    for start, end in compacted:
        if end - start + 1 <= max_hunk_lines:
            capped.append((start, end))
        else:
            capped.append((start, min(end, start + max_hunk_lines - 1)))
    return capped
